...
def align_shape(shape: bd.Shape,
                align: AlignT,
                ) -> bd.Shape:
    '''
    Returns a copy of the shape that has been moved to the alignment.
    '''
    def get_align_offset(shape: bd.Shape,
                         align: AlignT,
        ...
        for i in range(3):
            if align[i] == bd.Align.MIN:
                ...
                align_offset.append(
                    -(shape.bounding_box().min.to_tuple()[i]
                      + shape.bounding_box().max.to_tuple()[i]) / 2
                )
    ...
    align_location = bd.Location(
        get_align_offset(shape=shape, align=align)
    )
...
def combine_shapes(shapes: List[bd.Shape],
                   distance: float,
                   add_sprue: bool,
                   sprue_radius: float = 1.0,
                   sprue_offset: Optional[bd.Vector] = None,
                   align: AlignT = (
                       bd.Align.MIN, bd.Align.MIN, bd.Align.MIN
                   ),
                   ) -> bd.Shape:
